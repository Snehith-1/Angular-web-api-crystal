        ...
        public void DaGetPagedetails(MdlFacebook values)
        {
            try
            {
                ...
                if (responseAddress.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    {
                        var imagelist = objMdlFacebookMessageResponse.posts.data.Take(10);
                    ...
                    {
                        var videoslist = objMdlFacebookMessageResponse.videos.data.Take(10);
                ...
                msSQL = "select a.facebookmain_gid,a.post_id, a.post_url, DATE_FORMAT( a.postcreated_time, '%d-%m-%Y') AS postcreated_time,a.post_type,a.caption,a.views_count , ifnull(count(b.comment_message),0) as comment_message  " +
                        "  from  crm_smm_facebookpage a left join crm_smm_facebookpagedtl b on b.facebookmain_gid=a.facebookmain_gid   group by a.post_id order by a.postcreated_time desc;";
...
ex.Message.ToString() + "***********" + values.message.ToString() + "*****Query****" + msSQL + "*******Apiref********", "SocialMedia/ErrorLog/Facebook/" + "Log" + DateTime.Now.ToString("yyyy - MM - dd HH") + ".txt");
        ...
        public void DaUploadImage(HttpRequest httpRequest, string user_gid, result objResult)
        {
            ...
            catch (Exception ex)
            {
                ...
                objcmnfunctions.LogForAudit("*******Date*****" + DateTime.Now.ToString("yyyy - MM - dd HH: mm:ss") + "***********" + $"DataAccess:{System.Reflection.MethodBase.GetCurrentMethod().Name} " + " * **********" +
                ex.Message.ToString() + "***********" + objResult.message.ToString() + "*****Query****" + msSQL + "*******Apiref********", "SocialMedia/ErrorLog/Facebook/" + "Log" + DateTime.Now.ToString("yyyy - MM - dd HH") + ".txt");
            }
            //return true;
        ...
        public void DaScheduleUploadImage(HttpRequest httpRequest, string user_gid, result objResult)
        {
            ...
            //DateTime dateTime = DateTime.ParseExact(schedule_at, "yyyy-MM-dd HH:mm:ss", null);
                ...
                if (httpRequest.Files.Count > 0)
                {
                    ...
                    for (int i = 0; i < httpFileCollection.Count; i++)
                    {
                        ...
                        if (FileExtension == ".jpg" | FileExtension == ".png" | FileExtension == ".jpeg" | FileExtension == ".gif" | FileExtension == ".JPG" | FileExtension == ".JPEG" | FileExtension == ".JIFF" | FileExtension == ".TIFF" | FileExtension == ".PNG" | FileExtension == ".GIF" | FileExtension == ".JFIF" | FileExtension == ".svg" | FileExtension == ".jfif")
                        {
                            ...
                            ServicePointManager.SecurityProtocol = SecurityProtocolType.Ssl3 | SecurityProtocolType.Tls12 | SecurityProtocolType.Tls11 | SecurityProtocolType.Tls;
                            var client = new RestClient("https://graph.facebook.com");
                            var request = new RestRequest("/v19.0/" + getfacebookcredentials.page_id + "/photos?access_token=" + getfacebookcredentials.access_token, Method.POST);
...

                        {
                            ...
                            ServicePointManager.SecurityProtocol = SecurityProtocolType.Ssl3 | SecurityProtocolType.Tls12 | SecurityProtocolType.Tls11 | SecurityProtocolType.Tls;
                            var client = new RestClient("https://graph.facebook.com");
                            var request = new RestRequest("/v19.0/" + getfacebookcredentials.page_id + "/videos?access_token=" + getfacebookcredentials.access_token, Method.POST);
                            request.AddHeader("Content-Type", "application/json");
                            string filePath = Path.Combine(ConfigurationManager.AppSettings["blob_imagepath1"],
                                                            final_path, msdocument_gid + FileExtension +
                                                            ConfigurationManager.AppSettings["blob_imagepath2"] + '&' +
        ...
        public void DaGetschedulelogdetails(MdlFacebook values)
        {
            try
            {
                         result result = new result();
                ...
                msSQL = " select post_id,post_type,post_url,caption,schedule_at,postcreated_time,file_name from crm_smm_tfacebookschedule where posted_flag !='Y' ";
                dt_datatable = objdbconn.GetDataTable(msSQL);
