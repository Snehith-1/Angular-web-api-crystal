    ...
    public class DaPmrTrnPurchaseOrder
    {
        ...
        public void DaGetPurchaseOrderSummary(MdlPmrTrnPurchaseOrder values)
        {
            try
            {
                ...
                " bscc_flag,po_type,a.created_date from pmr_trn_tpurchaseorder a" +
                ...
                " where 1=1 and a.workorderpo_flag='N'  and a.purchaseorder_status <>'PO Amended'group by a.created_date desc ";
...
" where branch_gid  ='" + branch_gid + "' ";
        ...
        public void DaGetOnChangeVendor(string vendorregister_gid, MdlPmrTrnPurchaseOrder values)
        {
            try
            {
                
                msSQL = "  select a.vendorregister_gid,b.address1, b.address2, b.city, b.state, b.postal_code, b.fax,a.contactperson_name, a.vendor_companyname,a.payment_terms, " +
...
"  left join acp_mst_tvendorregister d on d.vendorregister_gid = a.vendorregister_gid  " +
...
" where a.module_gid = 'MKT' and c.templatetype_gid='2' ";
        ...
        public Dictionary<string, object> DaGetPurchaseOrderRpt(string purchaseorder_gid, MdlPmrTrnPurchaseOrder values)
        {
            ...
            msSQL = "SELECT  a.product_gid, b.hsn_number, a.purchaseorderdtl_gid, " +
                            "CONCAT('£ ', FORMAT(a.product_price, 2)) AS product_price, " +
                            "CONCAT('£ ', ROUND(a.product_price, 2)) AS product_price_round, a.qty_ordered, a.discount_percentage, " +
                            "CONCAT('£ ', FORMAT(a.discount_amount, 2)) AS discount_amount, a.tax_name, a.tax_amount, a.tax_name2, " +
                            "a.tax_amount2, a.tax_name3, a.tax_amount3, a.tax_percentage, a.tax_percentage2, a.tax_percentage3, " +
                            "e.addon_amount, a.needby_date AS ExpDate, a.product_remarks, (a.qty_ordered * a.product_price) AS created_by, " +
                            "a.needby_date, CONCAT('£ ', FORMAT((((a.qty_ordered * a.product_price) - a.discount_amount) + a.tax_amount + " +
                            "a.tax_amount2 + a.tax_amount3), 2)) AS product_total, SUBSTRING_INDEX(a.product_code, '/', 1) AS Lproduct_code, " +
                            "SUBSTRING_INDEX(a.product_code, '/', -1) AS Rproduct_code, a.product_code, a.product_name, c.productgroup_name, " +
                            "a.productuom_name, CASE WHEN a.display_field_name IS NULL THEN a.product_name " +
                            "WHEN a.display_field_name IS NOT NULL THEN a.display_field_name END AS display_field_name, " +
                            "(SUM((a.qty_ordered * a.product_price) - a.discount_amount) + a.tax_amount + a.tax_amount2 + a.tax_amount3) AS sum_product_total, " +
                            "FORMAT(e.discount_amount, 2) AS dis_amount, FORMAT(e.total_amount, 2) AS grand_total, d.productuom_name, e.total_amount, " +
                            "CONCAT(a.tax_name, ' : £', a.tax_amount) AS DataColumn36, e.payment_terms AS DataColumn37 " +
            ...
            msSQL = "SELECT e.purchaseorder_date, e.delivery_days, " +
                        ...
                        "CONCAT(UPPER(ConvertAmountinWords(e.total_amount)), ' POUNDS ONLY') AS DataColumn10, " +
            ...
            msSQL = "SELECT a.address1, a.branch_name, a.city, a.gst_no, a.state, a.postal_code, a.contact_number, a.email, a.email_id, a.branch_gid, a.branch_logo, a.tin_number, a.cst_number " +
        ...
        public void DaGetFromMailDropdown(MdlPmrTrnPurchaseOrder values)
        {
            try
            {
                ...
                msSQL = "select company_gid,pop_username from adm_mst_tcompany";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                var getModuleList = new List<GetFromMailDropdownlist>();
                if (dt_datatable.Rows.Count != 0)
                {
                    foreach (DataRow dt in dt_datatable.Rows)
                    {
                        getModuleList.Add(new GetFromMailDropdownlist
                        {
                            company_gid = dt["company_gid"].ToString(),
                            pop_username = dt["pop_username"].ToString(),

                        });
                        values.GetFromMailDropdownlist = getModuleList;
                    }
                }
                dt_datatable.Dispose();
            }
            ...
            {
                values.message = "Exception occured while getting Email!";
                objcmnfunctions.LogForAudit("*******Date*****" + DateTime.Now.ToString("yyyy - MM - dd HH: mm:ss") + "***********" +
              $"DataAccess: {System.Reflection.MethodBase.GetCurrentMethod().Name}" + "***********" +
              ex.Message.ToString() + "***********" + values.message.ToString() + "*****Query****" +
              msSQL + "*******Apiref********", "ErrorLog/Purchase/" + "Log" +
