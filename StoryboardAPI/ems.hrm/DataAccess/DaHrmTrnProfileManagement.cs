        ...
        public void DaUpdatePersonalDetails(string user_gid, personaldetails values)
        {
            try
            {
                ...
                msSQL = " update hrm_mst_temployee set " +
                        ...
                        " marital_status = '" + values.martial_edit + "'," +
                        " updated_by = '" + user_gid + "'," +
    ...
    public void DaGetNominationSummary(MdlHrmTrnProfileManagement values, string employee_gid)
        {
            try
            {
                ...
                msSQL = " select familydtl_gid,familymember_name,date_format(dob, '%d-%m-%Y') as dob,age,mobile_no,relationship,residing_with,residing_address,nominee_for from hrm_trn_tformfamilydetails where employee_gid='" + employee_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
