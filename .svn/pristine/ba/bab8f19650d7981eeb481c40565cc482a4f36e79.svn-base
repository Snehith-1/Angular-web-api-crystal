...
namespace ems.inventory.DataAccess
{
    ...
    public class DaImsTrnDeliveryordersummary
    {
        ...
        string msEmployeeGID, lsemployee_gid, lsentity_code, lsdelivery_quantity, lsdesignation_code, lsuom_gid, lsbranch_gid, lsCode, msGetGid, msGetGid1, msGetPrivilege_gid, msGetModule2employee_gid, msstockdtlGid;
        ...
        public void DaGetImsTrnDeliveryorderSummary(MdlImsTrnDeliveryordersummary values)
        {
            ...
            {                
                ...
                " a.customer_name, customer_branchname,a.customer_contactperson, directorder_status,delivery_status, " +
        ...
        public void DaGetProductdelivery(string salesorder_gid, MdlImsTrnDeliveryordersummary values)
        {
            ...
            string branch_gid = objdbconn.GetExecuteScalar(msSQL);
                ...
                msSQL = " select a.salesorderdtl_gid,a.delivery_quantity, c.design_no,c.color_name,a.salesorder_gid,a.product_gid," +
                    " date_format(a.product_requireddate, '%d-%m-%Y') as product_requireddate,z.productgroup_gid,z.productgroup_name,a.product_name,a.uom_gid,a.uom_name,a.qty_quoted, " +   
                  ...
                  " ims_trn_tstock m where m.stock_flag = 'Y' and m.product_gid = b.product_gid and m.branch_gid = '"+ branch_gid +"' and " +
        ...
        public void DaIssueFromStock(string product_gid, string salesorder_gid, MdlImsTrnDeliveryordersummary values)
        {
            ...
            msSQL = " select DATE_FORMAT(a.created_date, '%d-%m-%Y') as created_date,a.stock_gid,a.product_gid,a.display_field,a.uom_gid,a.reference_gid," +
        ...
        public void DaPostSelectIssueQtySubmit(string employee_gid, IssuedQty_list values)
        {
...

            {
               ...
               msSQL= " update smr_trn_tsalesorderdtl set delivery_quantity='" + values.total_amount + "', " +
                    "product_delivered='"+values.total_amount+"'"+
                             " where salesorder_gid='" + values.salesorder_gid + "'";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);


                msSQL = " update ims_trn_tstock set " +
                  " stock_qty = stock_qty - '" + values.total_amount + "' " +
                  " where stock_gid='" + values.stock_gid + "'";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);

                msSQL = "select delivery_quantity from smr_trn_tsalesorderdtl where salesorder_gid='"+values.salesorder_gid+"'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
        ...
        public void DaPostDeliveryOrderSubmit(MdlImsTrnDeliveryorder values, string employee_gid)
        {
            ...
            msSQL = " select a.salesorderdtl_gid,a.salesorder_gid,a.product_gid,date_format(a.product_requireddate, '%d-%m-%Y') as product_requireddate,z.productgroup_gid,z.productgroup_name,a.product_name,a.uom_gid,a.uom_name,a.qty_quoted," +
                    " a.display_field,a.product_delivered,a.product_price, a.discount_percentage,format(a.discount_amount,2) as discount_amount," +
                    " format(a.tax_amount,2) as tax_amount,format(a.tax_amount2,2) as tax_amount2,format(a.tax_amount3,2) as tax_amount3, " +
                    " a.tax_name,a.tax_name2,a.tax_name3, a.price,b.stockable,a.customerproduct_code, " +
                    " (select ifnull(sum(m.stock_qty)+sum(m.amend_qty)-sum(m.damaged_qty)-sum(m.issued_qty)-sum(m.transfer_qty),0) as available_quantity from " +
                    "  ims_trn_tstock m " +
                    ...
                    " left join pmr_mst_tproduct b on a.product_gid=b.product_gid " +
                    " left join pmr_mst_tproductgroup z on b.productgroup_gid=z.productgroup_gid" +
                    " WHERE a.product_gid = '" + values.product_gid + "'and a.salesorderdtl_gid = '" + values.salesorderdtl_gid + "'";
            ...
            //    lsproduct = objOdbcDataReader["product_name"].ToString();
            //    lsuomname = objOdbcDataReader["uom_name"].ToString();
            //    lsuom_gid = objOdbcDataReader["uom_gid"].ToString();
            //    objOdbcDataReader.Close();
            //}
                if (dt_datatable.Rows.Count != 0)
                ...
                if (objOdbcDataReader.HasRows)
            {
                ...
                lsstockquantity = objOdbcDataReader["stock_quantity"].ToString();
                objOdbcDataReader.Close();
            }
            ...
            mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
                ...
                {
                    objOdbcDataReader.Read();
                    lsdelivery_quantity = objOdbcDataReader["delivery_quantity"].ToString();
                ...
                msSQL = " update smr_trn_tsalesorderdtl set delivery_quantity='0' " +
                            " where salesorder_gid='" + values.salesorder_gid + "'";
                mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
