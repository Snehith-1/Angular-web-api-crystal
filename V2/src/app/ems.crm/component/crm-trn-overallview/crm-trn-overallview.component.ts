...
raisequotation() {
  ...
  const lead2campaign_gid = AES.encrypt(this.lead2campaign_gid, secretKey).toString();
  const leadbankcontact_gid = AES.encrypt(this.leadbankcontact_gid, secretKey).toString();
  this.router.navigate(['/crm/CrmTrnQuoteadd',leadbank_gid,lead2campaign_gid,leadbankcontact_gid,lspage]);
} 
...
viewinvoice(params: any){
  ...
  this.lspage = AES.encrypt(lspage, secretKey).toString();
  const leadbank_gid = AES.encrypt(this.leadbank_gid, secretKey).toString();
  const lead2campaign_gid = AES.encrypt(this.lead2campaign_gid, secretKey).toString();
  const leadbankcontact_gid = AES.encrypt(this.leadbankcontact_gid, secretKey).toString();
  this.router.navigate(['/einvoice/Invoice-View', invoice_gid,leadbank_gid,lead2campaign_gid,leadbankcontact_gid,lspage])
}
